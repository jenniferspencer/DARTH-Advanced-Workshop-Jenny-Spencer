...
v.position              <- ifelse((v.behave=="msmo"| v.behave=="msmw"), c(sample(c("Receptive",          
                                                                        ...
                                                                        replace= TRUE, 
                                                                         prob=c(p.pos_r, p.pos_i, p.pos_v))), NA)
...
edges_msm               <- ((n.msmo + n.msmw)* 0.74 * 0.5)       #an average of 0.74 relationships per person -> edges is this times 0.5
edges_msm_c             <- ((n.msmo + n.msmw)* 0.08)             # 8% of all MSM or MSMW should be in concurrent nodes
edges_msm_v             <- ((n.msmo + n.msmw)* 0.74 * p.pos_v) #an average of 0.74 relationships per versatile person 
...
edges_msw               <- ((n.mswo + n.wsmo)* 0.81 * 0.5)       #an average of 0.81 relationships per person -> edges is this times 0.5
edges_msw_c             <- ((n.mswo + n.wsmo)* 0.04)             #4% of all MSW should be in concurrent nodes
