...
####--------------------------------------------------- Death Module -----------------------------------------------------####
  ...
  dat$attr$age          <- get.vertex.attribute(dat$nw, "age")
  dat$attr$age.actv     <- get.vertex.attribute(dat$nw, "age.actv")
  dat$attr$active2      <- get.vertex.attribute(dat$nw, "active2")
    } 
    
      
    ## Attributes ##  
    active              <- dat$attr$active  #record whether active ID
    active2             <- dat$attr$active2  #record whether active ID
    age                 <- dat$attr$age     #record age
  ...
  drates_of_eligM       <- mort.ratesM[ages_of_eligM]
    ...
    if (nDeathsM > 0) {
      ...
      dat$nw            <- set.vertex.attribute(dat$nw, "active2", c(rep(0, nDeathsM)), idsDeathsM)
  ...
  b.func                <- function(dat, at) {
   ...
   dat$attr$age         <- get.vertex.attribute(dat$nw, "age")
   dat$attr$age.actv    <- get.vertex.attribute(dat$nw, "age.actv")
   dat$attr$p_total     <- get.vertex.attribute(dat$nw, "p_total")
   dat$attr$agefirst    <- get.vertex.attribute(dat$nw, "agefirst")
  ...
  dat$attr$active2      <- c(dat$attr$active2,     rep(1,   nBirths))  #everyone is active
  ...
  dat$attr$infTime      <- c(dat$attr$infTime,     rep(NA,  nBirths))  #this exists at the start but should be missing here 
  
  dat$attr$p_total      <- c(dat$attr$p_total,     rep(0,  nBirths))   #empy partner count variable
  dat$attr$agefirst     <- c(dat$attr$agefirst,    rep(NA,  nBirths))   #empy partner count variable
  ...
  dat$nw                <- set.vertex.attribute(dat$nw, "sex", c(rep("M", nBirths)), newNodes)
       ...
       dat$nw           <- set.vertex.attribute(dat$nw, "sex", c(rep("M", nBirths)), newNodes)
       dat$nw           <- set.vertex.attribute(dat$nw, "active2", c(rep(1, nBirths)), newNodes)
 ...
 #Determine when each person has 
  age.actv.new          <- rep(9, nBirths) #rpert(nBirths, min=8, mode=15, max=25, shape=10)
...
#Prevalence genital   
 ...
 # browser()
   ...
   #Get the full edgelist
   full_el              <- as.data.frame(dat$nw)
   
  #Identify new partnerships 
  new_rel               <-  full_el[which(full_el$duration == 1), ]

  #Create counter for head partners and then tail partners
    res1                <- new_rel %>% group_by(head, duration) %>% summarise(Freq=n())
    res2                <- new_rel %>% group_by(tail, duration) %>% summarise(Freq=n())

    
  #Add to exisiting counter   
  dat$attr$p_total[
    res1$head]          <- dat$attr$p_total[res1$head] + res1$Freq
  
  dat$attr$p_total[
    res2$tail]          <- dat$attr$p_total[res2$tail] + res2$Freq
  
  
  #If they don't have an agefirst yet, add it- first for tail partners than for head partners 
  agefirst[
    res1$head[
      is.na(agefirst[
        res1$head])]]   <- age[res1$head[is.na(agefirst[res1$head])]]
  
  agefirst[
    res2$tail[
      is.na(agefirst[
        res2$tail])]]   <- age[res2$tail[is.na(agefirst[res2$tail])]]
  
  dat$attr$agefirst     <- agefirst
  
  set.vertex.attribute(dat$nw, "p_total", dat$attr$p_total) 
  
  set.vertex.attribute(dat$nw, "agefirst", dat$attr$agefirst) 
